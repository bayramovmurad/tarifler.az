...
const UserRecipes = () => {
  ...
  // Yüklənmə vəziyyəti üçün müasir UI
  if (isLoading) {
    return (
    ...
    <div className="relative mt-6">
      {userRecipes?.length > 0 ? (
        ...
        <div className="text-center py-12 bg-slate-900/50 rounded-2xl border border-slate-800 border-dashed">
          <p className="text-slate-400 text-sm">Hələ ki, heç bir resept əlavə etməmisiniz.</p>
        </div>
      )}
      ...
      {showUpdate && (
        <div
          ...
          onClick={() => setShowUpdate(false)} // Arxa fona kliklədikdə bağlanır
        >
          <div
            className="relative w-full max-w-lg"
            onClick={(e) => e.stopPropagation()} // İçəri kliklənəndə bağlanmasın deyə
          >
